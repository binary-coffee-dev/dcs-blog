...
module.exports = {
  ...
  migrate: async () => {
    ...
    await strapi.plugins['users-permissions'].models.permission
      .updateMany({controller: 'post', action: ['find', 'count'], role: writerRole.id}, {$set: {enabled: true}});
    await strapi.plugins['users-permissions'].models.permission
      .updateMany({controller: 'post', action: ['find', 'count'], role: staffRole.id}, {$set: {enabled: true}});
    await strapi.plugins['users-permissions'].models.permission
      .updateMany({controller: 'post', action: ['find', 'count'], role: adminRole.id}, {$set: {enabled: true}});
  },
};
