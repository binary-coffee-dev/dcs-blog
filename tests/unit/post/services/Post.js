...
describe('Post service', function () {
  ...
  before(() => {
    postService = strapi.service('api::post.post');
  });

  it('should convert the text to dash base', function () {
    const name = postService.getNameFromTitle('  This is an example  ');
    expect(name.substring(0, name.length - 5)).to.equal('this-is-an-example');
  });

  it('should convert vowels with accent', () => {
    const name = postService.getNameFromTitle('Thís   áis éan óexample ú ñ äëïöü');
